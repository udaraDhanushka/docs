...
move_file() {
    ...
    # Check if target already exists and is a directory
    if [ -d "$full_target_path" ]; then
        # Target is an existing directory, move file into it
        full_target_path="$full_target_path/$(basename "$file_path")"
    elif [ -e "$full_target_path" ]; then
        # Target exists but is not a directory (it's a file)
        echo "Error: Target '$target_path' already exists as a file!"
        return 1
    else
        # Target doesn't exist - determine intent
        # If target ends with '/', treat as directory
        if [[ "$target_path" == */ ]]; then
            # User wants to create a directory and move file into it
            mkdir -p "$full_target_path"
            full_target_path="$full_target_path/$(basename "$file_path")"
        else
            # Check if parent directory needs to be created for file move
            target_dir=$(dirname "$full_target_path")
            if [ ! -d "$target_dir" ]; then
                read -p "Parent directory '$target_dir' doesn't exist. Create it? (Y/N): " create_parent
