...
- Practice Git: init, commit, branch, merge, rebase, pull requests
...
- Choose one major cloud platform: AWS, GCP, Azure
...
### Problem Solving
- Understand the problem fully before writing any code — describe it in plain words and confirm you are solving the root cause, not a symptom
- Break the problem into the smallest independent steps and test each one before moving to the next
- Search for existing solutions in documentation, forums, and repositories before building from scratch
- Write a short plain-language plan before coding — clarity on paper prevents bugs in code
- Debug systematically: reproduce the issue reliably, isolate where it occurs, understand why, then fix and verify with a test
- When stuck for more than 30 minutes, explain the problem out loud or ask for help — describing it often reveals the solution
- Reflect briefly after every solved problem to identify the root cause and build pattern recognition over time
